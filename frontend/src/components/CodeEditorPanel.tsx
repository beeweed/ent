...
const CodeEditor: React.FC<EditorProps> = ({ file, onChange }) => {
  ...
  if (!file) {
    return (
      <div data-design-id="code-editor-empty-state" className="flex-1 flex flex-col items-center justify-center text-zinc-500 bg-[#1e1e1e] h-full">
  ...
  return (
    <div data-design-id="code-editor-monaco-wrapper" className="h-full w-full overflow-hidden">
      <Editor
        height="100%"
        width="100%"
        language={language}
  ...
  return (
    ...
    >
      ...
      <input
        ref={inputRef}
        type="text"
        className="bg-transparent text-white outline-none w-full text-xs placeholder-zinc-500"
...
const FileTree: React.FC<FileTreeProps> = ({
  ...
  const handleItemClick = (_e: React.MouseEvent, item: FileSystemItem) => {
    if (item.type === 'folder') {
        ...
        <div key={item.id}>
          <div 
            className={`
              ...
              ${item.id === activeFileId ? 'bg-blue-900/40 text-blue-300' : 'text-zinc-400 hover:bg-[#2a2a2b] hover:text-zinc-200'}
            `}
            style={{ paddingLeft: `${level * 12 + 8}px` }}
...
const PreviewPanel: React.FC<PreviewPanelProps> = ({
  ...
  return (
    <div data-design-id="preview-panel" className="h-full flex flex-col bg-[#1e1e1e]">
      <div className="h-10 bg-[#2d2d2d] border-b border-[#333] flex items-center px-3 gap-2 flex-shrink-0">
        ...
        <span className="text-xs text-zinc-400 flex-shrink-0">Preview</span>
        
        <div className="flex items-center ml-2">
          <span className="text-xs text-zinc-500 mr-1">Port:</span>
          <input
            type="text"
            value={port}
            onChange={(e) => setPort(e.target.value)}
            className="w-16 bg-[#1e1e1e] border border-[#444] rounded px-2 py-0.5 text-xs text-zinc-300 focus:outline-none focus:border-blue-500"
            placeholder="3000"
          />
        </div>
        ...
        <div className="flex items-center ml-auto space-x-1">
          <button
            ...
            className={`p-1.5 rounded ${deviceMode === 'desktop' ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:text-white hover:bg-[#444]'}`}
            title="Desktop view"
          >
            <Monitor size={12} />
          </button>
          <button
            ...
            className={`p-1.5 rounded ${deviceMode === 'tablet' ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:text-white hover:bg-[#444]'}`}
            title="Tablet view"
          >
            <Tablet size={12} />
          </button>
          <button
            ...
            className={`p-1.5 rounded ${deviceMode === 'mobile' ? 'bg-blue-600 text-white' : 'text-zinc-400 hover:text-white hover:bg-[#444]'}`}
            title="Mobile view"
          >
            <Smartphone size={12} />
          </button>
        ...
        <div className="flex items-center space-x-1 ml-2">
          <button
            ...
            disabled={!previewUrl}
            className="p-1.5 text-zinc-400 hover:text-white hover:bg-[#444] rounded disabled:opacity-50"
      ...
      <div className="h-8 bg-[#252526] border-b border-[#333] flex items-center px-3">
        <div className="flex-1 bg-[#1e1e1e] border border-[#444] rounded px-2 py-1 flex items-center">
          {previewUrl ? (
            <>
              <span className="text-green-400 text-xs mr-1">https</span>
              <span className="text-xs text-zinc-300 truncate">{previewUrl}</span>
            </>
          ) : (
      ...
      <div className="flex-1 overflow-hidden flex items-center justify-center bg-[#0f0f10] p-4">
        {error ? (
          ...
          <div className="text-center">
            <Globe size={48} className="mx-auto mb-4 opacity-20 text-zinc-500" />
            <p className="text-sm text-zinc-500">Enter a valid port to preview</p>
          </div>
        )}
      </div>

      <div className="h-6 bg-[#252526] border-t border-[#333] flex items-center px-3 text-[10px] text-zinc-500">
...
const SandboxControls: React.FC<SandboxControlsProps> = ({
  ...
  return (
    <div data-design-id="sandbox-controls" className="bg-[#252526] border-b border-[#333]">
      <div 
      ...
      >
        <div className="flex items-center space-x-2">
          ...
          {isConnected && sandboxId && (
            <span className="text-[10px] text-zinc-500 bg-[#1e1e1e] px-2 py-0.5 rounded">
      ...
      {isExpanded && (
        <div className="px-4 pb-4 space-y-3">
          <div className="space-y-1">
            <label className="text-[10px] text-zinc-500 uppercase tracking-wider flex items-center">
            ...
            <div className="relative">
              <input
                ...
                className="w-full bg-[#1e1e1e] border border-[#444] rounded px-3 py-2 pr-10 text-sm text-zinc-300 
                  placeholder-zinc-600 focus:outline-none focus:border-blue-500 disabled:opacity-50 disabled:cursor-not-allowed"
              />
              <button
                type="button"
            ...
            {!isConnected ? (
              <button
                ...
                className="flex-1 flex items-center justify-center space-x-2 bg-green-600 hover:bg-green-700 
                  disabled:bg-zinc-700 disabled:cursor-not-allowed text-white rounded py-2 px-4 text-sm font-medium
                  transition-colors"
              >
                {isConnecting ? (
                  <>
          ...
          {isConnected && sandboxId && (
            <div className="p-2 bg-[#1e1e1e] rounded border border-[#333]">
              <p className="text-[10px] text-zinc-500 uppercase tracking-wider mb-1">Sandbox ID</p>
              <p className="text-xs text-zinc-300 font-mono break-all">{sandboxId}</p>
            </div>
          )}
        </div>
      )}
      ...
      <div className="h-8 bg-[#2d2d2d] border-b border-[#333] flex items-center justify-between px-1 flex-shrink-0">
        <div className="flex items-center flex-1 overflow-x-auto no-scrollbar">
          ...
          <button
            ...
            className="flex items-center justify-center w-7 h-7 text-zinc-500 hover:text-white hover:bg-[#444] rounded ml-1 flex-shrink-0"
            title="New Terminal"
          >
            <Plus size={14} />
          </button>
        ...
        <div className="flex items-center space-x-1 ml-2 flex-shrink-0">
          <button
            ...
            className={`text-[10px] px-2 py-0.5 rounded ${autoSync ? 'bg-green-600/20 text-green-400' : 'bg-zinc-600/20 text-zinc-400'}`}
...
export const CodeEditorPanel: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleMove = (e: MouseEvent | TouchEvent) => {
      ...
      if (isResizing === 'sidebar') {
        ...
        setBottomPanelHeight(Math.max(80, Math.min(400, newHeight)));
      }
    };
  ...
  return (
    <div 
      ref={containerRef} 
      data-design-id="code-editor-panel"
      className="flex h-full w-full bg-[#1e1e1e] text-zinc-300 font-sans overflow-hidden"
    ...
    >
      ...
      {isSidebarOpen && (
        <>
          <div 
            data-design-id="code-editor-sidebar"
          ...
          >
            ...
            {isConnected && (
              <div className="px-3 py-2 border-b border-[#333]">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-xs text-zinc-400 uppercase tracking-wide">File Sync</span>
                  {isSyncing && (
            ...
            <div className="h-10 flex items-center justify-between px-3 border-b border-[#333]">
              <span className="text-xs font-bold tracking-wider text-zinc-400 uppercase">Explorer</span>
              <div className="flex space-x-2">
                 <button 
                  onClick={() => handleStartCreating('root', 'folder')}
                  className="hover:text-white text-zinc-500" title="New Root Folder"
                >
                   <FolderPlus size={14}/>
                 </button>
                 <button 
                  onClick={() => handleStartCreating('root', 'file')}
                  className="hover:text-white text-zinc-500" title="New Root File"
                >
                   <FilePlus size={14}/>
                 </button>
              </div>
          ...
          <div
            ...
            className={`w-1 cursor-col-resize flex-shrink-0 flex items-center justify-center group touch-none
      ...
      <div className="flex-1 flex flex-col relative min-w-0">
        <div className="h-10 bg-[#2d2d2d] flex items-center justify-between px-3 border-b border-[#1e1e1e] flex-shrink-0">
            ...
            <div className="flex space-x-1 overflow-x-auto no-scrollbar mask-gradient flex-1">
              {openFiles.map(fileId => {
                  ...
                  <div
                      ...
                      className={`
                      flex items-center px-3 py-1 rounded-t text-xs cursor-pointer select-none min-w-[80px] max-w-[160px] flex-shrink-0
                      ${isActive ? 'bg-[#1e1e1e] text-blue-400 font-medium' : 'bg-[#2d2d2d] text-zinc-500 hover:bg-[#333] hover:text-zinc-300'}
                      `}
                  >
          ...
          <div className="flex items-center space-x-2 ml-3">
             ...
             <button 
                ...
                className={`p-1.5 rounded ${isBottomPanelOpen ? 'bg-[#444] text-green-400' : 'text-zinc-500 hover:text-white'}`}
             ...
             <button 
                ...
                className={`p-1.5 rounded ${isRightPanelOpen ? 'bg-[#444] text-blue-400' : 'text-zinc-500 hover:text-white'}`}
        ...
        <div className="flex-1 flex overflow-hidden">
          <div ref={editorAreaRef} className="flex-1 flex flex-col min-w-0">
            <div className="flex-1 overflow-hidden" style={{ minHeight: isBottomPanelOpen ? '80px' : '100%' }}>
              <CodeEditor 
                  file={activeFile} 
            ...
            {isBottomPanelOpen && (
              <>
                <div
                  ...
                  className={`h-1 cursor-row-resize flex-shrink-0 flex items-center justify-center group touch-none
          ...
          {isRightPanelOpen && (
            <>
              ...
              >
                <div className="h-8 bg-[#2d2d2d] border-b border-[#333] flex items-center px-2 flex-shrink-0">
                  <button
                    className="px-3 py-1 text-xs rounded bg-[#1e1e1e] text-blue-400"
                  >
        ...
        <div className="h-5 bg-[#007acc] text-white text-[10px] flex items-center px-3 justify-between select-none flex-shrink-0">
          ...
          <div className="flex items-center space-x-3">
            <span>{activeFile ? getLanguageFromFileName(activeFile.name) : 'Plain Text'}</span>
            <span>UTF-8</span>
            {sandboxId && (
